...
const screenshotDir = resolve(root, process.env.FARMTACT_INFEASIBLE_SCREENSHOT_DIR || 'apps/web/screenshots/infeasible')
const reportPath = resolve(root, process.env.FARMTACT_INFEASIBLE_REPORT || 'reports/infeasible_browser.json')
...
const report = {
  ...
  checks: [], failures: [], screenshots: [], intercepted: { scenarios: [], quests: [], conversations: [], providers: [] },
}
...
async function runViewport(browser, width) {
  ...
  const pageErrors = [], consoleErrors = [], browserRequests = []
  ...
  const before = await (await context.request.get(`${baseURL}/api/v1/bootstrap`)).json()
  ...
  const cash = lab.locator('.range-control').filter({ hasText: 'Available cash' }).locator('input')
  await cash.fill('50')
  ...
  const after = await (await context.request.get(`${baseURL}/api/v1/bootstrap`)).json()
  ...
  check(`${width}px browser has no console errors`, consoleErrors.length === 0, consoleErrors)
  check(`${width}px loaded bootstrap over the real API`, browserRequests.some(url => new URL(url).pathname === '/api/v1/bootstrap'))
  await context.close()
}
...
try {
  ...
  for (const width of [390, 1280]) await runViewport(browser, width)
} catch (error) {
