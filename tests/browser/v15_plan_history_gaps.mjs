...
try{
 ...
 await click('Review demand changes');await page.getByLabel('Cash available (SGD)',{exact:true}).waitFor();
 const draft={tentative_orders:[],future_demand:[],seasonal:[],order_changes:[],reservations:[],capacity:{cash_sgd:-1}};
 await page.getByLabel('Cash available (SGD)',{exact:true}).fill('-1');await click('Review');await click('Create proposal');await page.getByRole('heading',{name:'Proposal and recalculation',exact:true}).waitFor();await click('Apply & recalculate');await page.getByRole('alert').waitFor();
 ...
 await page.getByText('Import saved settings',{exact:true}).click();await page.getByLabel('Planning assumptions JSON').fill(JSON.stringify(valid));
