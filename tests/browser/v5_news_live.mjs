/**
 * Isolated deployed-News variant of v5_news.mjs.
 *
 * The successful live run used DOMContentLoaded rather than the original
 * networkidle/30-second navigation, 60-second action/navigation bounds, a
 * three-second stale-response observation window, and waited for route disposal.
 * Assertions and application requests otherwise remain the candidate suite's.
 */
...
const state=process.env.FARMTACT_BROWSER_STATE||'/tmp/farmtact-v5-live-state-browser.json'
const output=process.env.FARMTACT_NEWS_REPORT||'reports/v5/news_live.json'
...
try{
  ...
  await page.goto(`${base}/${edition}/`,{waitUntil:'domcontentloaded'})
  ...
  await mkdir('apps/web/screenshots/v5-news-live',{recursive:true})
  await panel.screenshot({path:'apps/web/screenshots/v5-news-live/news-390.png'})
