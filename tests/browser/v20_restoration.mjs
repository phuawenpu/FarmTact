...
const root = resolve(new URL("../..", import.meta.url).pathname),
  base = (process.env.BASE_URL || "http://127.0.0.1:4199").replace(
    /\/$/,
    "",
  ),
  report = {
    status: "RUNNING",
    base_url: `${base}${process.env.APP_PATH || "/"}`,
...
try {
  await mkdir("/tmp/v20-browser", {recursive:true});
  ...
  check('public entry restores V12 experience in current edition',await page.locator(`[data-experience="v12"][data-edition="${process.env.EXPECTED_EDITION || 'v20'}"]`).count()===1);
  check(
  ...
  const phaseLabels=await page.locator('.flow-rail button span').allTextContents();
  check('original seven stages retained in order',['Observe','Discuss','Decide','Approve','Act','Verify','Replan'].every((label,i)=>phaseLabels[i]?.startsWith(label)),phaseLabels);
  ...
  check('all seven Council specialists visible', await page.locator('.role-card').count()===7);
  await page.locator('.proposal-card').filter({hasText:'Lean'}).click();
  check('strategy comparison selects candidate locally', (await page.locator('.proposal-card.is-selected h3').textContent())==='Lean');
  await page.locator('.proposal-card').filter({hasText:'Balanced'}).click();
  ...
  const workflowState=await page.evaluate(async()=>fetch(`/api/v1/farm-workflow`).then(response=>response.json()));
  ...
  if(alreadyApproved){
    ...
    approvedState=await page.evaluate(async()=>fetch(`/api/v1/farm-workflow`).then(response=>response.json()));
  }
  ...
  await page.reload({waitUntil:'domcontentloaded'});
  await page.getByRole('heading',{name:/See the farm/}).waitFor();
  const reloadedState=await page.evaluate(async()=>fetch(`/api/v1/farm-workflow`).then(response=>response.json()));
  ...
  for (const width of [360, 390, 430, 1280]) {
    ...
    const nav=page.locator(width===1280?'.side-rail nav':'.thumb-nav');
    check(`${width}px has all eight named sections`,await nav.getByRole('button').count()===8);
    ...
    const file = resolve(process.env.ARTIFACT_DIR || "/tmp/v20-browser", `restoration-${width}.png`);
  ...
  for(const [label,heading] of [['Farm',/Singapore/],['Farm tools',/Turn today/],['Crops',/Recipes with receipts/],['Data',/See every number/],['Outcomes',/Learn from every turn/],['Setup',/Bring the farm/],['Council research',/Talk through a farm decision/]]) {
    await nav.getByRole('button',{name:label,exact:true}).click();
    if(label==='Farm') await page.locator('#farm-world-title').waitFor();
    else await page.getByRole('heading',{name:heading}).first().waitFor();
    check(`named section ${label} opens actual workspace`,true);
    if(label==='Crops') {
      const card=page.locator('.crop-profile-card').first();
      await card.click();
      await page.getByRole('dialog').waitFor();
      await page.getByText('Loading evidence profile…').waitFor({state:'hidden'});
      await page.keyboard.press('Escape');
      await page.getByRole('dialog').waitFor({state:'hidden'});
      check('crop profile closes with Escape',await page.getByRole('dialog').count()===0);
      const focusReturned=await card.evaluate(el=>document.activeElement===el);
      report.ux_observations.push({name:'crop profile returns source focus',pass:focusReturned,classification:'original V12 accessibility behavior; later improvement'});
    }
  }
  await nav.getByRole('button',{name:'Plan',exact:true}).click();
  await page.getByRole('heading',{name:/See the farm/}).waitFor();
  check('named navigation returns to original durable planning workflow',await page.locator(`[data-task-id="${target.id}"]`).count()===1);
  ...
  await mkdir(resolve(root, "reports/v20"), { recursive: true });
  await writeFile(
    resolve(root, process.env.REPORT_PATH || (process.env.STAGED_SOURCE ? "/tmp/v20-browser/staged-restoration.json" : "reports/v20/local-browser.json")),
