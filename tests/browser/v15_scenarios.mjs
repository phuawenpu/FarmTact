...
try{
 const stateFile='/tmp/farmtact-v15-scenarios-'+new URL(base).port+'.json';let saved=false;try{await access(stateFile);saved=true}catch{}
 const context=await browser.newContext({viewport:{width:430,height:900},...(saved?{storageState:stateFile}:{})});page=await context.newPage();
 await page.goto(base);await page.locator('.ic-shell').waitFor();await context.storageState({path:stateFile});
 ...
 await click('Open');if(await page.getByRole('button',{name:'Create scenario',exact:true}).isVisible().catch(()=>false))await click('Create scenario');else await click('New branch');await page.getByLabel('Name',{exact:true}).fill('V15 parent branch');
 ...
 await page.getByLabel('Name',{exact:true}).fill('V15 child branch');await page.getByLabel('yield percent',{exact:true}).fill('95');await page.getByLabel('Batch ID (for crop delay)',{exact:true}).fill(session.farm.batches[0].id);await click('Review');
