...
try {
  ...
  const qualitativeContext = firstReply.locator('.tool-references')
  await qualitativeContext.locator(':scope > summary').click()
  check('legacy tool references render as explicitly qualitative frozen context', await qualitativeContext.getByText('Qualitative context references').isVisible() && await qualitativeContext.getByText('14 Sept 2026').isVisible() && await qualitativeContext.getByText(/-\$42\.50/).isVisible())
