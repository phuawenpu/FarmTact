...
## Final resolution in immutable V16

The reopened semantic gate is resolved in published V16, source `3487faf`.
`reports/v16/final-semantics.json` passes 49 assertions across every core stage and
labelled missing/mixed-fact cases; `explanation-layout.json` independently verifies
API facts, containment, focus, zoom and reduced motion in 97 checks at four widths.
All four full real journeys pass 27 checks each. Exact hosted acceptance passes
20 operator and 28 browser checks. Public verification passes 67 browser, 13
isolation and nine preservation checks. See `reports/v16/implementation.md` and
`reports/v16/completion-audit.md`. V15 remains immutable, including its recorded
failure; V16 is the sole current experience. The full rewrite goal is complete,
with representative-user usability explicitly unverified.
