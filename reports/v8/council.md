...
| --- | --- | --- |
| AI-01 | `packages/ai_quality.py` scores persisted validation, disjoint reference types, required topic groups, expected relationships, grounded facts, unsupported quantity and known contradictions separately. `scripts/deepseek_quality_e2e.py` requires the exact role sequence and message count for planning, direct, invite, conversation Council and research. It can combine an HTTP conversation with read-only planning/research artifacts and records source provenance. The retained pre-fix quality baseline has 18 cases: workflow integrity passes, while 10 cases fail for unsupported replies, missing research references, informal weather abstention, or observed semantic defects. | `PASS_AUTOMATED` covers only the listed deterministic checks. The independent semantic-review dimension remains pending unless a reasoned per-case review artifact is supplied; it is informative, not a human release gate. |
...
| AI-08 | The truthful sequence remains six independent specialists over the same frozen context followed by the chair. Only the chair receives bounded prior claims, including evidence status and validation issues. Budget remains seven roles plus two shared repairs. Model-facing mission references use deterministic, disjoint short aliases; exact server maps restore canonical references before the unchanged validator and renderer. Audits retain supplied maps and original returned aliases, while public claims remain canonical. A format repair is allowed only for authored-number/count prose or a typed fact placed in qualitative refs; the original rejected response, issues, usage and audit remain stored. The repair must preserve nonnumeric lexical meaning, relationship, actions, evidence and highlights. Unknown references and unsupported semantics are never retried. | No challenge round or phantom debate was added. Its value must be measured before spending more calls. |
...
/tmp/farmtact-docs-venv/bin/python scripts/deepseek_quality_e2e.py \
  --state /tmp/farmtact-v8-ai-trial.json \
  --planning-report reports/v8/planning-final.json \
  --research-replay reports/v8/research-advisor-replay.json \
...
Latest focused package verification: **250 passed, 1 skipped, 1 PostgreSQL concurrency case deselected** in 57.74 seconds. The selected groups cover all DeepSeek, Council, injection-boundary, provider-isolation, conversation, research-harness and AI-quality tests. Root runs the complete suite against its dedicated test database after integration.

The pre-fix actual replay recorded 805,878 summed prompt tokens across 10 persisted conversation replies and 64,756 prompt tokens for the research reply. The v4 context now projects role-relevant facts, limits prior turns and evidence rows, recursively bounds nested values, and rejects a serialized model context above 120,000 characters before inference. Post-fix provider token use remains to be measured; the implementation does not infer a reduction from the offline character-bound tests.
