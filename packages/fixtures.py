...
def synthetic_farm(cutoff=None):
    ...
    for ci,r in enumerate(recipes):
        for w in range(8):
            orders.append(dict(id=f'order-{ci}-{w}',crop_id=r['crop_id'],booked_at=cutoff-timedelta(days=7),due_date=day+timedelta(days=w*7+6),quantity_kg=str(22+(ci%2)*4+(w%3)*2),price_sgd_per_kg=str(6+ci)))
        for w in range(12):
            end=day-timedelta(days=7*(12-w))
            history.append(dict(crop_id=r['crop_id'],week=end,available_at=datetime.combine(end,datetime.min.time(),timezone.utc)+timedelta(days=1),ordered_kg=str(27+ci*2+(w%4)*2)))
