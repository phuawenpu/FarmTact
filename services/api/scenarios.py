...
def install_routes(app,tenant):
    ...
    def create(body:ScenarioRequest,request:Request):
        ...
        with store.transaction(t) as c:
            ...
            source=conversation['_snapshot'] if conversation else (parent['input_snapshot'] if parent else store.latest_farm(t))
    ...
    def compare(request:Request,ids:str):
        ...
        if len({s['baseline_hash'] for s in rows})!=1:raise HTTPException(409,'Branches must share the same frozen baseline')
