...
One Fly Machine (`2871575b4544d8`, app `farmtact`, region `sin`, 4 shared vCPUs,
4096 MB, `min_machines_running = 1`, autostop off) serves the public site. It runs
two containers from the **same pinned image digest**
`registry.fly.io/farmtact@sha256:e05b00a575983dfc79d83a41563c3cfe762579475e57d885df414d6705ecdee1`:
an internet-facing `gateway` on port 8080 and the `v22` edition on port 8102, which
has no public service attached. The gateway proxies to the edition over the private
`farmtact-local-v22.flycast:8102` hop.

The edition digest matches the `v22` entry in `config/releases/registry.json` and
the source commit tagged `farmtact-v22` (`ac6c84d09d515eac51802138baceabf84a3c01a8`).
The Fly health check polls `GET /api/v1/health` every 30 seconds and currently
returns `{"status":"ok","edition":"v22","source_commit":"ac6c84d0…"}`. One encrypted
3-GB volume (`vol_vdejexpzm8ydn5x4`, snapshot retention 7) is mounted at `/persist`;
each container bind-mounts only its own root-owned subtree to `/data` before
unmounting the shared parent, so editions keep isolated database, cache and progress
state. PostgreSQL uses private Unix sockets and the configured
`FARMTACT_DATABASE_URL` contains no password or network host.

Runtime secrets are Fly secrets injected per container:
`FARMTACT_CONTROL_SECRET` on both containers and `DEEPSEEK_API_KEY` on the edition
container only. No credential, database URL or environment value is served by an
HTTP endpoint; publication refuses non-private IPs and deploys with `--no-public-ips`.

Root `fly.toml` describes an older single-container bootstrap (mount `/data`,
1 shared vCPU / 2 GB, `force_https = false`, no container block) and **does not**
describe this topology. The canonical machine configuration is generated by
`scripts/shared_host_config.py` and identified by `config/hosting/shared.json`;
never run a generic `fly deploy` against the live app.

Known hardening gaps in the running image are recorded in
[the security inventory](docs/security.md) and
[the September 2026 live audit](reports/security_v22_audit.md): the private-only
`/_control/*` gate is not an authorization boundary on its own, the gateway installs
no abuse middleware, and no HSTS header is sent.
