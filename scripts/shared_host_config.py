...
def machine_config(registry, volume, *, active=None, staged=None, retirement_operator=False, public=False, origin='https://farmtact.fly.dev'):
    ...
    for name, image in [('gateway', gateway_image), *images.items()]:
        ...
        containers.append(dict(name=name,image=image,env=env,
            ...
            # Latest-only gateway readiness probes the edition too. Start the
            # edition after gateway startup, not after its dependent HTTP probe.
            healthchecks=[dict(name=name+'-ready',http=dict(port=port,method='GET',path='/api/v1/health',
                **({'headers':[dict(name='Fly-Client-IP',values=['127.0.0.1'])]} if gateway and int(ids[-1][1:])>=14 else {})),
                interval=15,timeout=5,grace_period=60,success_threshold=1,failure_threshold=3)],
            **({} if gateway else {'depends_on':[dict(name='gateway',condition='started' if int(ids[-1][1:])>=14 else 'healthy')]})))
