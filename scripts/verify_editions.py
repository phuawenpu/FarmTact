...
try:
    with httpx.Client(base_url=args.url, timeout=30, follow_redirects=True) as client:
        ...
        active_payload = client.get('/api/releases').json()
        ...
        if retired:
            ...
            check('Retired edition read returns 410 with active links',
                  gone.status_code == 410 and all(f'/{edition}/' in gone.text for edition in active))
