    ...
    {data && <>
      ...
      <details className="news-browser">
        ...
        {!frozen && <div className="news-filters">
          <label>Region<select aria-label="Region" value={geography} onChange={event => setGeography(event.target.value)}><option value="all">Singapore + region</option><option value="singapore">Singapore</option><option value="regional">Regional</option></select></label>
          <label>Time<select aria-label="Time" value={period} onChange={event => setPeriod(event.target.value)}><option value="all">All available dates</option><option value="recent">Published in last 7 days</option><option value="future">Announced future events</option><option value="ongoing">Ongoing dated events</option><option value="historical">Older publications</option></select></label>
          <label>Crop in headline<select aria-label="Crop in headline" value={crop} onChange={event => setCrop(event.target.value)}><option value="">All crops + general context</option>{['caixin', 'pak_choi', 'kailan', 'lettuce', 'bayam', 'kangkong', 'kale', 'mustard_greens', 'malabar_spinach', 'sweet_potato_leaves'].map(id => <option key={id} value={id}>{id.replaceAll('_', ' ')}</option>)}</select></label>
        </div>}
        ...
        <div className="news-records">{data.records.map(record => {
          ...
          return <article key={record.id}>
            ...
            <dl><div><dt>Published</dt><dd>{date(record.published_at)}</dd></div><div><dt>Retrieved</dt><dd>{date(record.retrieved_at)} · {record.retrieval_freshness === 'fresh' ? 'fresh cache' : 'stale cache'}</dd></div>
              <div><dt>Observation</dt><dd>{date(record.observed_at)}</dd></div><div><dt><CalendarDays size={13}/> Event starts</dt><dd>{date(record.event_start_at)}</dd></div>{record.event_end_at && <div><dt>Event ends</dt><dd>{date(record.event_end_at)}</dd></div>}</dl>
