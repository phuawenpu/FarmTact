...
export default function IntegratedApp(){
 ...
 {tool==='records'?<><div hidden={recovery}><Records onClose={close} onOpenPlan={()=>setRecovery(true)}/></div>{recovery&&<Plan onClose={()=>setRecovery(false)}/>}</>:tool==='knowledge'?<Knowledge onClose={close}/>:tool==='experiments'?<><div hidden={research}><Experiments onClose={close} onResearch={()=>setResearch(true)}/></div>{research&&<Research onClose={()=>setResearch(false)}/>}</>:tool==='plan'?<Plan onClose={close}/>:<History onClose={close}/>}
 </Suspense>}/>
}
