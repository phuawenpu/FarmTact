...
export function IntegratedKnowledge({ onClose, initialTarget }: { onClose: () => void; initialTarget?: KnowledgeTarget }) {
  ...
  const [selectedCategory, setSelectedCategory] = useState<"home" | "crops" | "sources" | "advisers">("home");
  ...
  useEffect(() => {
    ...
    if (initialTarget.kind === "crops" || initialTarget.kind === "sources" || initialTarget.kind === "advisers") {
      setIndex(0); setSelectedCategory(initialTarget.kind);
      requestAnimationFrame(() => document.querySelector<HTMLElement>(`[data-knowledge-category-pick="${initialTarget.kind}"]`)?.focus({ preventScroll: false }));
      return;
    }
  ...
  return <section className="ik-shell" tabIndex={-1} onKeyDown={keyboard}>
    ...
    <div className="ik-deck" onPointerDown={(event) => { pointer.current = event.clientX; }} onPointerUp={pointerUp}>
      <BoundCard card={knowledgeCard} className="ik-card" aria-live="polite" aria-label={`Knowledge card ${activeIndex + 1} of ${cards.length}`}>
        {active.kind === "index" && <KnowledgeIndex crops={bootstrap.crops} sources={sourceCards} query={cropQuery} setQuery={setCropQuery} category={selectedCategory} setCategory={setSelectedCategory} open={openFromIndex}/>}
...
function KnowledgeIndex({ crops, sources, query, setQuery, category, setCategory, open }: { crops: Crop[]; sources: Source[]; query: string; setQuery: (value: string) => void; category: "home" | "crops" | "sources" | "advisers"; setCategory: (value: "home" | "crops" | "sources" | "advisers") => void; open: (kind: Card["kind"], id: string) => void }) {
  ...
  return <><span className="ik-kicker">Library index · reading makes no provider call</span><h2>Choose what you need</h2><p>Crops and sources explain planning assumptions. Specialists answer an explicit question. Planning Council reviews the current frozen alternatives; the separate Council research study is under Experiments.</p>
    <nav className="ik-category-picks" aria-label="Knowledge categories"><button data-knowledge-category-pick="crops" className={category === "crops" ? "is-selected" : ""} onClick={() => setCategory("crops")}><b>Crops</b><span>{crops.length} profiles</span></button><button data-knowledge-category-pick="sources" className={category === "sources" ? "is-selected" : ""} onClick={() => setCategory("sources")}><b>Sources</b><span>{sources.length} records</span></button><button data-knowledge-category-pick="advisers" className={category === "advisers" ? "is-selected" : ""} onClick={() => setCategory("advisers")}><b>Specialists</b><span>{ADVISORS.length} roles</span></button></nav>
    {category === "home" && <p className="ik-category-prompt">Choose a category above, or open a saved discussion or Planning Council review below.</p>}
    {category === "crops" && <section className="ik-category" data-knowledge-category="crops"><header><div><h3>Crop atlas</h3><span>{matching.length} shown</span></div><p>Recipes, representative stage art, cautions and evidence limits.</p></header><label>Find a crop<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, ID or alias"/></label><div className="ik-picker is-crops">{matching.map((crop) => { const art = cropStageArt[crop.id]?.[0]; return <button key={crop.id} data-knowledge-pick={`crop:${crop.id}`} onClick={() => open("crop", crop.id)}>{art && <img src={`/art/crops/${art}`} alt={`${crop.label} reference illustration`}/>}<span>{crop.label}</span><small>{crop.recipe ? `${crop.recipe.cycle_days} day cycle` : "Accounting profile only"}</small></button>; })}</div>{!matching.length && <p>No crop profile matches this search.</p>}</section>}
    {category === "sources" && <section className="ik-category" data-knowledge-category="sources"><header><div><h3>Sources</h3><span>{sources.length} records</span></div><p>Freshness, coverage, licence and admitted facts.</p></header><div className="ik-picker">{sources.map((source) => <button key={source.id} data-knowledge-pick={`source:${source.id}`} onClick={() => open("source", source.id)}><span>{source.name}</span><small>{words(source.availability_status || source.status)}</small></button>)}</div></section>}
    {category === "advisers" && <section className="ik-category" data-knowledge-category="advisers"><header><div><h3>Specialists</h3><span>{ADVISORS.length} roles</span></div><p>Opening is read-only. Only Submit question, Invite specialist or Convene Council invokes the provider.</p></header><div className="ik-picker is-people">{ADVISORS.map((advisor) => <button key={advisor.id} data-knowledge-pick={`advisor:${advisor.id}`} onClick={() => open("advisor", advisor.id)}><img src={`/art/advisors/${advisor.id}.svg`} alt=""/><span>{advisor.name}</span><small>{advisor.role}</small></button>)}</div></section>}
...
function CouncilCard({ session, status }: { session: PlanningSession | null; status: string }) {
  ...
  const resultId = (session as (PlanningSession & { result_id?: string }) | null)?.result_id || "Unavailable";
  return <><span className="ik-kicker">Planning Council · advisory decision brief</span><h2>Review the frozen alternatives</h2><p>Use this card to inspect evidence-bound advice, then return to the exact plan card that opened it. Local strategies remain available when review is partial, withheld, or unavailable.</p>
    <dl className="ik-facts"><Fact label="Session revision" value={session?.revision ?? "Unavailable"}/><Fact label="Frozen result" value={resultId}/><Fact label="Council status" value={words(status)}/></dl>
    ...
    {!findings.length && <p>{session?.result?.strategies?.length ? "Ready for explicit review from the emphasized action. This submission uses the existing validated gateway; browsing this card does not call a provider." : "Calculate planning alternatives before requesting review."}</p>}
    {!!findings.length && <section className="ik-findings"><h3>Recorded findings</h3>{findings.map((raw, index) => { const finding = raw as Record<string, unknown>; const facts = Array.isArray(finding.rendered_facts) ? finding.rendered_facts.filter((item): item is import("../lib/game").RenderedFact => !!item && typeof item === "object" && "reference" in item) : [];
      const nextActions = Array.isArray(finding.proposed_actions) ? finding.proposed_actions.filter((item): item is Record<string, unknown> => !!item && typeof item === "object") : [];
      return <article key={String(finding.id || index)}><header><b>{words(finding.role || `Council finding ${index + 1}`)}</b><span>{words(finding.validation_status || finding.evidence_status || finding.status || "recorded")}</span></header><p>{String(finding.rendered_interpretation || finding.statement || finding.summary || "No interpretation supplied.")}</p><FrozenFactEvidence factRefs={Array.isArray(finding.fact_refs) ? finding.fact_refs.map(String) : []} renderedFacts={facts} validationStatus={String(finding.validation_status || "")} evidenceStatus={String(finding.evidence_status || finding.status || "")}/>{nextActions.length > 0 && <p className="ik-next-action"><b>Advisory next action:</b> {nextActions.map((action) => `${words(action.control || "action")} ${action.value == null ? "" : String(action.value)} ${action.unit == null ? "" : String(action.unit)} · ${words(action.status || "not applied")}`).join("; ")}</p>}</article>; })}</section>}
    <p className="ik-boundary">Evidence-reference checks do not establish that qualitative prose is true. Council review never changes assumptions, approves work, or authorizes a physical operation. Use Back to return to the same plan card and choose the next reviewed action.</p></>;
}
